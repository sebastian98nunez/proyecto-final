...
using namespace std;
int MAXCOLS=7; //eje x...
int p1=1000;//INICIALIZAMMOS LAS PUNTUACIONES
int p2=1000;
...
void menu_inicio(){
    ...
    if(mouse_x>96 && mouse_x <326 && mouse_y>143 && mouse_y <169  ){//NUEVO JUEGO
        ...
        if(mouse_b & 1){
            while(!key[KEY_C]){
                blit(INSTRUCCIONES_,buffer,0,0,0,0,440,360);
                pantalla();
            }
        }
    }
...
int main(){
    ...
    INSTRUCCIONES_=load_bitmap("INSTRUCCIONES.bmp",NULL);
